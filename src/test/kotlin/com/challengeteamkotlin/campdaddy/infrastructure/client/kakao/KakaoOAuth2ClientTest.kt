...
class KakaoOAuth2ClientTest(
    ...
    Given("getAccessToken 테스트") {
        When("getAccessToken이 틀리다면") {
            every { kakaoOAuth2Client.getAccessToken(any()) } throws RetrievalFailureException(MemberErrorCode.ACCESS_TOKEN_RETRIEVAL_FAILURE)
            Then("예외가 발생한다") {
                ...
                getAccessTokenExceptionErrorCode shouldBe MemberErrorCode.ACCESS_TOKEN_RETRIEVAL_FAILURE
            }
        }
    }
    ...
    Given("retrieveUserInfo 테스트") {
        When("가져온 토큰과 유저의 정보가 일치 하지 않으면") {
            every { kakaoOAuth2Client.retrieveUserInfo(failedAccessToken) } throws RetrievalFailureException(MemberErrorCode.USER_INFO_RETRIEVAL_FAILURE)
            Then("예외가 발생한다") {
                ...
                retrieveUserInfoExceptionErrorCode shouldBe MemberErrorCode.USER_INFO_RETRIEVAL_FAILURE
            }
        }
    }
})
